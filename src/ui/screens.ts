...
export async function sendOrEditUi(api:TelegramApi,db:D1Database,user:User,text:string,keyboard:InlineKeyboard|undefined,replyRows:string[][],sourceMessageId?:number){
  ...
  if(sourceMessageId){
    try{await api.editMessageText(user.id,sourceMessageId,text,opts);await db.prepare('UPDATE users SET ui_message_id=? WHERE id=?').bind(sourceMessageId,user.id).run();await refreshReplyKeyboard(api,user.id,user.language);return sourceMessageId;}catch{}
  }
  if(user.ui_message_id){
    try{await api.editMessageText(user.id,user.ui_message_id,text,opts);await refreshReplyKeyboard(api,user.id,user.language);return user.ui_message_id;}catch{}
  }
  ...
  await refreshReplyKeyboard(api,user.id,user.language);
  return msg.message_id;
}

async function refreshReplyKeyboard(api:TelegramApi,userId:number,lang:Lang){
  const rows=[[t(lang,'menu_chat'),t(lang,'images')],[t(lang,'change_model'),t(lang,'my_chats')],[t(lang,'tools'),t(lang,'account')]];
