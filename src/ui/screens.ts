...
export async function sendOrEditUi(api:TelegramApi,db:D1Database,user:User,text:string,keyboard:InlineKeyboard|undefined,replyRows:string[][],sourceMessageId?:number){
  ...
  if(sourceMessageId){
    try{await api.editMessageText(user.id,sourceMessageId,text,opts);await db.prepare('UPDATE users SET ui_message_id=? WHERE id=?').bind(sourceMessageId,user.id).run();return sourceMessageId;}catch{}
  }
  if(user.ui_message_id){
    try{await api.editMessageText(user.id,user.ui_message_id,text,opts);return user.ui_message_id;}catch{}
  }
