...
export async function sendOrEditUi(api:TelegramApi,db:D1Database,user:User,text:string,keyboard:InlineKeyboard|undefined,replyRows:string[][]=[],sourceMessageId?:number){
  ...
  if(target){
    try{
      ...
      if(/message is not modified/i.test(String(err)))return target;
      try{await api.deleteMessage(user.id,target)}catch{}
    }
  }
  const msg=await api.sendMessage(user.id,text,{...opts, ...(replyRows.length?{reply_markup:rk(replyRows)}:{})});
...
export async function sendHomeBannerUi(api:TelegramApi,db:D1Database,env:Env,user:User,sourceMessageId?:number){
  ...
  if(banner){
    try{
      ...
      // Keep the persistent Telegram keyboard in sync without leaving an extra
      // visible message in the chat.
      try{
        const kb=await api.sendMessage(user.id,'\u2063',{reply_markup:rk(replyRows)});
        try{await api.deleteMessage(user.id,kb.message_id)}catch{}
      }catch{}

      await db.prepare('UPDATE users SET ui_message_id=? WHERE id=?').bind(msg.message_id,user.id).run();
      return msg.message_id;
    }catch(err){
      // A Telegram file_id can become invalid after a bot/media migration or
      // when an administrator deletes/replaces the source media. Clear only
      // the stale banner value and fall back to the normal home UI.
      console.error('home_banner_send_error',String(err));
      await repo.setSetting(db,'home_banner_file_id','');
    }
  }
  return sendOrEditUi(api,db,user,t(user.language,'welcome'),rows,replyRows);
}
...
export async function chatsScreen(api:TelegramApi,db:D1Database,user:User,page=1,sourceMessageId?:number){
 const limit=Number((await repo.getSetting(db,'active_chat_limit'))??5); const totalCount=await repo.countChats(db,user.id,false); const size=5; const total=Math.max(1,Math.ceil(totalCount/size)); const p=Math.min(Math.max(1,page),total); const chats=await repo.listChats(db,user.id,false,p,size);
...
export async function archiveScreen(api:TelegramApi,db:D1Database,user:User,page=1,sourceMessageId?:number){
 ...
 const text=`<b>${t(user.language,'archive_title')}</b>\n\n${t(user.language,'archive_hint')}\n\n${t(user.language,'archive_count',{count:totalCount,limit:Number((await repo.getSetting(db,'archive_chat_limit'))??15)})}`;
 return sendOrEditUi(api,db,user,text,rows,[],sourceMessageId);
}
...
export async function imageQualityScreen(api:TelegramApi,db:D1Database,env:Env,user:User,modelKey:string,state:any,sourceMessageId?:number){const m=await repo.modelByKey(db,modelKey);if(!m)return;const o=imageOptions(m);const rows=await Promise.all(o.qualities.map(async q=>{const price=await repo.imageQualityCost(db,String(q));return [{text:`${state.quality===q?'✅ ':''}${q}${price>0?` · +${price}`:''}`,callback_data:`image:setquality:${encodeURIComponent(modelKey)}:${encodeURIComponent(q)}`}] }));rows.push([{text:t(user.language,'back'),callback_data:`image:settings:${encodeURIComponent(modelKey)}`}]);return sendOrEditUi(api,db,user,`✨ <b>Качество</b>\n\nВыберите качество изображения.`,rows,[],sourceMessageId)}
