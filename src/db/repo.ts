...
import { rolePromptForKey } from '../ai/persona-prompts';
...
export async function ensureChat(
  ...
  // Recover gracefully if the stored current_chat_id is missing or points
  // to an archived chat: prefer an existing active chat instead of creating
  // an extra active dialog on every recovery path.
  const active = await listChats(db, user.id, false, 1, 1);
  if (active[0]) {
    const chat = active[0];
    await db.prepare(
      'UPDATE users SET current_chat_id=?, last_model_key=? WHERE id=?'
    ).bind(chat.id, chat.model_key, user.id).run();
    return chat;
  }
...
export async function createChat(
  ...
  const {active:activeLimit}=await getChatLimits(db);
  const id=uuid();
  const now=nowIso();
  const result=await db.batch([
    db.prepare(
      `INSERT INTO chats(
        id,user_id,seq,title,model_key,role_key,custom_role,is_archived,
        created_at,updated_at,last_continued_at
      )
      SELECT ?,?,seq,('Новый диалог №'||seq),?,?,NULL,0,?,?,?
      FROM (SELECT COALESCE((SELECT MAX(seq) FROM chats WHERE user_id=?),0)+1 AS seq)
      WHERE (SELECT COUNT(*) FROM chats WHERE user_id=? AND is_archived=0) < ?`
    ).bind(id,userId,modelKey,roleKey,now,now,now,userId,userId,activeLimit),
    db.prepare(
      `UPDATE users
       SET current_chat_id=?,last_model_key=?
       WHERE id=?
         AND EXISTS(SELECT 1 FROM chats WHERE id=? AND user_id=?)`
    ).bind(id,modelKey,userId,id,userId)
  ]);

  if(Number(result[0]?.meta?.changes??0)!==1)throw new Error('ACTIVE_CHAT_LIMIT');
  if(Number(result[1]?.meta?.changes??0)!==1)throw new Error('CHAT_CURRENT_UPDATE_FAILED');
...
export async function getSearchModel(db: D1Database,key:string,env?:Env):Promise<Model|null>{
  const model=await first<Model>(db.prepare("SELECT * FROM models WHERE model_key=? AND is_active=1 AND type='search'").bind(key));
  if(!model)return null;
  if(env && !providerConfigured(model,env))return null;
  return model;
}
...
export async function listSearchModels(
  ...
  // Only expose search models whose provider is actually configured. This
  // keeps the UI consistent with the real runtime availability.
  return models.filter(model => providerConfigured(model, env));
}
...
export async function getRole(
  ...
  return role ? {...role,prompt:rolePromptForKey(role.role_key,role.prompt)} : null;
}

export async function listRoles(
  db: D1Database
): Promise<Role[]> {
  const roles=await all<Role>(db.prepare('SELECT * FROM roles WHERE is_active=1 ORDER BY sort,name_ru'));
  return roles.map(r=>({...r,prompt:rolePromptForKey(r.role_key,r.prompt)}));
}
...
export async function getSettings(db:D1Database,keys:string[]):Promise<Record<string,string>>{
  ...
  const rows=await all<{key:string;value:string}>(db.prepare(`SELECT key,value FROM settings WHERE key IN (${placeholders(unique.length)})`).bind(...unique));
...
export async function saveMessages(
  ...
  await db.batch([
    ...
    db
      .prepare(
        'UPDATE chats SET updated_at=?,last_continued_at=? WHERE id=? AND user_id=?'
      )
      .bind(
        now,
        now,
...
export async function captureHoldAndSaveMessages(
  db: D1Database,
  holdId: string,
  chatId: string,
  userId: number,
  ...
  const now = nowIso();
  const expires = new Date(
    Date.now() + Math.min(24, ttlHours) * 3600 * 1000
  ).toISOString();
  const captureTransactionId = `${holdId}:capture`;

  const result = await db.batch([
    db.prepare(
      `UPDATE point_holds
       SET status='captured',finished_at=?
    ...
    db.prepare(
      ...
      FROM point_holds
      WHERE id=? AND status='captured' AND finished_at=?`
    ).bind(captureTransactionId, now, holdId, now),

    db.prepare(
      `INSERT INTO messages(
        id,chat_id,user_id,role,content,cost,created_at,expires_at
      ) VALUES(?,?,?,?,?,?,?,?)`
    ).bind(uuid(), chatId, userId, 'user', userText, 0, now, expires),

    db.prepare(
      `INSERT INTO messages(
        id,chat_id,user_id,role,content,cost,created_at,expires_at
      ) VALUES(?,?,?,?,?,?,?,?)`
    ).bind(uuid(), chatId, userId, 'assistant', assistantText, cost, now, expires),

    db.prepare(
      `UPDATE chats
       SET updated_at=?,last_continued_at=?
       WHERE id=? AND user_id=?`
    ).bind(now, now, chatId, userId)
  ]);
...
export async function updateChatModel(
  ...
  await db.batch([
    db
      .prepare(
        'UPDATE chats SET model_key=? WHERE id=? AND user_id=?'
      )
      .bind(
        modelKey,
        chatId,
...
export async function setChatRole(
  ...
  await db
    .prepare(
      'UPDATE chats SET role_key=?, custom_role=? WHERE id=? AND user_id=?'
    )
    .bind(
      roleKey,
      customRole,
...
export async function markChatContinued(db:D1Database,chatId:string,userId:number):Promise<void>{
  const now=nowIso();
  await db.prepare('UPDATE chats SET last_continued_at=?,updated_at=? WHERE id=? AND user_id=?').bind(now,now,chatId,userId).run();
}

export async function renameChat(db:D1Database,chatId:string,userId:number,title:string):Promise<void>{
  await db.prepare('UPDATE chats SET title=? WHERE id=? AND user_id=?').bind(title,chatId,userId).run();
}
...
export async function archiveChat(
  ...
  const existing=await getChat(db,chatId,userId);
  if(!existing)return false;
  if(existing.is_archived)return true;
  const {archive:limit}=await getChatLimits(db);
  ...
  const changed=await db.prepare(
    `UPDATE chats
       SET is_archived=1,archive_expires_at=?
     WHERE id=? AND user_id=? AND is_archived=0
       AND (SELECT COUNT(*) FROM chats WHERE user_id=? AND is_archived=1) < ?`
  ).bind(expires,chatId,userId,userId,limit).run();
  if(Number(changed.meta?.changes??0)!==1){
    const fresh=await getChat(db,chatId,userId);
    return Boolean(fresh?.is_archived);
  }
  await db.batch([
...
export async function unarchiveChat(db:D1Database,chatId:string,userId:number):Promise<boolean>{
  const target=await getChat(db,chatId,userId);
  if(!target||!target.is_archived)return false;
  const {active:limit}=await getChatLimits(db);
  const changed=await db.prepare(
    `UPDATE chats SET is_archived=0,archive_expires_at=NULL
     WHERE id=? AND user_id=? AND is_archived=1
       AND (SELECT COUNT(*) FROM chats WHERE user_id=? AND is_archived=0) < ?`
  ).bind(chatId,userId,userId,limit).run();
...
):Promise<{ok:boolean;notFound:boolean;limit:boolean;chat:Chat|null}>{
  const target=await getChat(db,chatId,userId);
  if(!target)return {ok:false,notFound:true,limit:false,chat:null};
  const {active:limit}=await getChatLimits(db);
  const modelKey=modelKeyOverride||target.model_key;
  const now=nowIso();
  const result=await db.batch([
    db.prepare(
      `UPDATE chats
       SET is_archived=0,archive_expires_at=NULL
       WHERE id=? AND user_id=?
         AND (is_archived=0 OR (is_archived=1 AND (SELECT COUNT(*) FROM chats WHERE user_id=? AND is_archived=0) < ?))`
    ).bind(chatId,userId,userId,limit),
    db.prepare(
      `UPDATE users
       SET current_chat_id=?,last_model_key=?,mode='chat',search_model_key=NULL
       WHERE id=?
         AND EXISTS(SELECT 1 FROM chats WHERE id=? AND user_id=? AND is_archived=0)`
    ).bind(chatId,modelKey,userId,chatId,userId),
    db.prepare(
      `UPDATE chats
       SET last_continued_at=?,updated_at=?,model_key=?
       WHERE id=? AND user_id=? AND is_archived=0`
    ).bind(now,now,modelKey,chatId,userId)
  ]);
  const changed=Number(result[0]?.meta?.changes??0)===1;
  if(!changed){
    return {ok:false,notFound:false,limit:true,chat:target};
  }
  const userChanged=Number(result[1]?.meta?.changes??0)===1;
  if(!userChanged){
    return {ok:false,notFound:false,limit:false,chat:target};
  }
  return {ok:true,notFound:false,limit:false,chat:await getChat(db,chatId,userId)};
}
...
export async function listChats(
  ...
  return all<Chat>(
    db
      .prepare(
        'SELECT * FROM chats WHERE user_id=? AND is_archived=? ORDER BY COALESCE(last_continued_at,updated_at,created_at) DESC,id DESC LIMIT ? OFFSET ?'
      )
      .bind(
        userId,
        archived ? 1 : 0,
...
export async function captureHold(
  ...
  const now=nowIso();
  const result=await db.batch([
    db.prepare(`UPDATE point_holds SET status='captured',finished_at=? WHERE id=? AND status='held'`).bind(now,holdId),
    db.prepare(`INSERT OR IGNORE INTO transactions(id,user_id,kind,free_amount,paid_amount,ref,created_at)
                SELECT ?,user_id,'capture',0,0,ref,?
                FROM point_holds WHERE id=? AND status='captured' AND finished_at=?`)
      .bind(`${holdId}:capture`,now,holdId,now)
  ]);
  if(Number(result[0]?.meta?.changes??0)>0)return true;
  return Boolean(await first(db.prepare("SELECT 1 as ok FROM point_holds WHERE id=? AND status='captured'").bind(holdId)));
}

export async function refundCapturedHold(db:D1Database,holdId:string):Promise<boolean>{
  const now=nowIso();
  const ref=`${holdId}:refund`;
  await db.batch([
    db.prepare(
      `UPDATE balances
       SET free_points=free_points+(SELECT free_amount FROM point_holds WHERE id=? AND status='captured'),
           purchased_points=purchased_points+(SELECT paid_amount FROM point_holds WHERE id=? AND status='captured'),
           updated_at=?
       WHERE user_id=(SELECT user_id FROM point_holds WHERE id=? AND status='captured')
         AND NOT EXISTS(SELECT 1 FROM transactions WHERE kind='refund' AND ref=?)`
    ).bind(holdId,holdId,now,holdId,ref),
    db.prepare(
      `INSERT OR IGNORE INTO transactions(id,user_id,kind,free_amount,paid_amount,ref,created_at)
       SELECT ?,user_id,'refund',free_amount,paid_amount,ref||':refund',?
       FROM point_holds WHERE id=? AND status='captured'`
    ).bind(uuid(),now,holdId),
    db.prepare(
      `UPDATE point_holds SET status='refunded',finished_at=?
       WHERE id=? AND status='captured'
         AND EXISTS(SELECT 1 FROM transactions WHERE kind='refund' AND ref=?)`
    ).bind(now,holdId,ref)
  ]);
  return Boolean(await first(db.prepare("SELECT 1 as ok FROM point_holds WHERE id=? AND status='refunded'").bind(holdId)));
}
...
export async function markOrderPaid(
  ...
  const providerPaymentId=`manual:${orderId}:${settlementId}`;

  if(plan.kind==='subscription'&&plan.duration_days){
    const existing=await first<{expires_at:string}>(db.prepare(
      'SELECT expires_at FROM subscriptions WHERE user_id=? AND expires_at>? ORDER BY expires_at DESC LIMIT 1'
    ).bind(order.user_id,now));
    const start=existing?new Date(existing.expires_at):new Date(now);
    const expiresAt=new Date(start.getTime()+plan.duration_days*86400000).toISOString();
    const dailyPoints=Number(plan.points)>0?Number(plan.points):Number((await getSetting(db,'free_points_subscriber'))??100);
    const resetHours=Math.max(1,Number((await getSetting(db,'free_period_hours'))??24));

    const batch=await db.batch([
      db.prepare(`UPDATE orders SET status='paid',paid_at=?,provider='manual',provider_order_id=? WHERE id=? AND status='pending'`).bind(now,providerPaymentId,orderId),
      db.prepare(`INSERT OR IGNORE INTO payments(id,order_id,provider,provider_payment_id,amount_minor,currency,status,created_at)
                  SELECT ?,?,?,?,?,?,'paid',?
                  WHERE EXISTS(SELECT 1 FROM orders WHERE id=? AND status='paid' AND paid_at=? AND provider_order_id=?)`)
        .bind(uuid(),orderId,'manual',providerPaymentId,order.amount_minor,order.currency,now,orderId,now,providerPaymentId),
      db.prepare(`INSERT OR IGNORE INTO subscriptions(id,user_id,plan_key,starts_at,expires_at,source,order_id,created_at)
                  SELECT ?,?,?,?,?,?,?,?
                  WHERE EXISTS(SELECT 1 FROM payments WHERE order_id=? AND provider='manual' AND provider_payment_id=? AND status='paid')`)
        .bind(uuid(),order.user_id,plan.plan_key,start.toISOString(),expiresAt,'purchase',orderId,now,orderId,providerPaymentId),
      db.prepare(`UPDATE balances SET free_points=?,free_reset_at=?,updated_at=? WHERE user_id=?
                  AND EXISTS(SELECT 1 FROM payments WHERE order_id=? AND provider='manual' AND provider_payment_id=? AND status='paid')`)
        .bind(dailyPoints,new Date(Date.now()+resetHours*3600000).toISOString(),now,order.user_id,orderId,providerPaymentId),
      db.prepare(`INSERT OR IGNORE INTO transactions(id,user_id,kind,free_amount,paid_amount,ref,created_at)
                  SELECT ?,?,'subscription_bonus',?,0,?,?
                  WHERE EXISTS(SELECT 1 FROM payments WHERE order_id=? AND provider='manual' AND provider_payment_id=? AND status='paid')`)
        .bind(uuid(),order.user_id,dailyPoints,orderId,now,orderId,providerPaymentId)
    ]);
    if(Number(batch[0]?.meta?.changes??0)!==1)return null;
    const paidOrder=await first<Order>(db.prepare('SELECT * FROM orders WHERE id=?').bind(orderId));
    return paidOrder?{order:paidOrder,kind:'subscription',points:dailyPoints,days:plan.duration_days,expiresAt}:null;
  }
  ...
  if(plan.kind==='points'){
    const batch=await db.batch([
      ...
      db.prepare(`INSERT OR IGNORE INTO payments(id,order_id,provider,provider_payment_id,amount_minor,currency,status,created_at)
                  ...
                  WHERE EXISTS(SELECT 1 FROM orders WHERE id=? AND status='paid' AND paid_at=? AND provider_order_id=?)`)
        .bind(uuid(),orderId,'manual',providerPaymentId,order.amount_minor,order.currency,now,orderId,now,providerPaymentId),
      db.prepare(`UPDATE balances SET purchased_points=purchased_points+?,updated_at=? WHERE user_id=?
                  AND EXISTS(SELECT 1 FROM payments WHERE order_id=? AND provider='manual' AND provider_payment_id=? AND status='paid')`)
        .bind(plan.points,now,order.user_id,orderId,providerPaymentId),
      db.prepare(`INSERT OR IGNORE INTO transactions(id,user_id,kind,free_amount,paid_amount,ref,created_at)
                  SELECT ?,?,'purchase',0,?,?,?
                  WHERE EXISTS(SELECT 1 FROM payments WHERE order_id=? AND provider='manual' AND provider_payment_id=? AND status='paid')`)
        .bind(uuid(),order.user_id,plan.points,orderId,now,orderId,providerPaymentId)
    ]);
    if(Number(batch[0]?.meta?.changes??0)!==1)return null;
    const paidOrder=await first<Order>(db.prepare('SELECT * FROM orders WHERE id=?').bind(orderId));
    return paidOrder?{order:paidOrder,kind:'points',points:plan.points}:null;
  }
...
export async function getChatLimits(db:D1Database):Promise<{active:number;archive:number}>{
  const settings=await getSettings(db,['active_chat_limit','archive_chat_limit']);
  return {active:Math.max(1,Number(settings.active_chat_limit??5)),archive:Math.max(1,Number(settings.archive_chat_limit??15))};
}
...
export async function getDialogMessageLimit(db:D1Database,userId:number):Promise<number>{
  const settings = await getSettings(db,[
    'chat_message_limit_free',
    'chat_message_limit_subscriber',
    'chat_message_limit_2y'
  ]);
  const free=Math.max(1,Number(settings.chat_message_limit_free??DEFAULT_FREE_DIALOG_MESSAGES));
  const subscriber=Math.max(1,Number(settings.chat_message_limit_subscriber??DEFAULT_SUBSCRIBER_DIALOG_MESSAGES));
  const twoYear=Math.max(1,Number(settings.chat_message_limit_2y??DEFAULT_TWO_YEAR_DIALOG_MESSAGES));
...
export async function grantSubscriptionPlan(db:D1Database,userId:number,planKey:string,source='admin'):Promise<{expiresAt:string;days:number;points:number}|null>{
  ...
  await db.batch([
    ...
    db.prepare('INSERT INTO transactions(id,user_id,kind,free_amount,paid_amount,ref,created_at) VALUES(?,?,?,?,?,?,?)').bind(uuid(),userId,'subscription_bonus',daily,0,`${source}:${planKey}:${nowS}:${uuid()}`,nowS)
  ]);
...
export async function imageQualityCost(db:D1Database,quality:string):Promise<number>{const defaults:Record<string,number>={low:1,standard:2,medium:2,high:3,ultra:5,hd:3};const q=String(quality||'standard').toLowerCase();return Math.max(0,Number((await getSetting(db,`image_quality_price_${q}`))??defaults[q]??0))}
export async function templateCost(db:D1Database,_id:string,_defaultCost=0):Promise<number>{return 0}

export async function cleanupExpiredArchivedChats(db:D1Database):Promise<void>{
  const now=nowIso();
  await db.prepare(
    `UPDATE chats
     SET archive_expires_at=datetime(updated_at,'+' || COALESCE((
       SELECT CASE WHEN EXISTS(SELECT 1 FROM subscriptions s WHERE s.user_id=chats.user_id AND s.expires_at>datetime('now') AND s.plan_key IN ('sub-1m','sub-3m','sub-6m','sub-1y','sub-2y')) THEN 48 ELSE 24 END
     ),24) || ' hours')
     WHERE is_archived=1 AND archive_expires_at IS NULL`
  ).run();
  await db.prepare('DELETE FROM chats WHERE is_archived=1 AND archive_expires_at IS NOT NULL AND archive_expires_at<=?').bind(now).run();
}

export async function switchToArchivedChat(db:D1Database,chatId:string,userId:number):Promise<boolean>{
  return unarchiveChat(db,chatId,userId);
}
