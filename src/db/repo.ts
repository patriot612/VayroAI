...
export async function archiveChat(
  ...
  if(Number(count?.count??0)>=limit)return false;
  const retention=await getArchiveRetentionHours(db,userId);
  const now=nowIso();
  const expires=new Date(Date.now()+retention*3600000).toISOString();
  await db.batch([
    db.prepare('UPDATE chats SET is_archived=1,archive_expires_at=? WHERE id=? AND user_id=?').bind(expires,chatId,userId),
...
export async function unarchiveChat(db:D1Database,chatId:string,userId:number):Promise<boolean>{
  ...
  if(Number(active?.count??0)>=limit)return false;
  await db.prepare('UPDATE chats SET is_archived=0,archive_expires_at=NULL WHERE id=? AND user_id=?').bind(chatId,userId).run();
  return true;
}

export async function deleteChat(db:D1Database,chatId:string,userId:number):Promise<void>{
 await db.batch([db.prepare('DELETE FROM messages WHERE chat_id=? AND user_id=?').bind(chatId,userId),db.prepare('DELETE FROM chats WHERE id=? AND user_id=?').bind(chatId,userId),db.prepare('UPDATE users SET current_chat_id=NULL WHERE id=? AND current_chat_id=?').bind(userId,chatId)]);
}
