...
export async function archiveChat(
  ...
  const limit=Number((await getSetting(db,'archive_chat_limit'))??15);
  ...
  await db.batch([
    db.prepare('UPDATE chats SET is_archived=1, updated_at=?, archive_expires_at=? WHERE id=? AND user_id=?').bind(now,expires,chatId,userId),
...
export async function unarchiveChat(
  ...
  const limit=Number((await getSetting(db,'active_chat_limit'))??5);
  ...
  await db.prepare('UPDATE chats SET is_archived=0,archive_expires_at=NULL,updated_at=? WHERE id=? AND user_id=?').bind(nowIso(),chatId,userId).run();
  return true;
}
