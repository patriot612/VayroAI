...
export async function adminBanner(api:TelegramApi,db:D1Database,user:User,sourceMessageId?:number){const current=await repo.getSetting(db,'home_banner_file_id');const text=`${t(user.language,'banner_admin')}\n\n${current?'✅ Баннер установлен.':'⚪ Баннер не установлен.'}`;return ui(api,db,user,text,[[{text:t(user.language,'banner_upload'),callback_data:'admin:banner_upload'}],[{text:t(user.language,'banner_preview'),callback_data:'admin:banner_preview'}],[{text:t(user.language,'banner_delete'),callback_data:'admin:banner_delete'}],back(user)],sourceMessageId)}
