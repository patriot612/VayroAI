...
async function parseOpenAIImageResponse(res:Response){const data:any=await res.json();if(!res.ok)throw new Error('IMAGE_API_ERROR');const item=data?.data?.[0];if(!item)throw new Error('IMAGE_EMPTY');if(item.b64_json){const bin=atob(item.b64_json);const out=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)out[i]=bin.charCodeAt(i);return {bytes:out.buffer,mime:String(item.media_type||mimeFromBytes(out.buffer)||'image/png')}}if(item.url){const r=await fetch(item.url);if(!r.ok)throw new Error('IMAGE_RESULT_FETCH_FAILED');const bytes=await r.arrayBuffer();return {bytes,mime:String(item.media_type||r.headers.get('content-type')||mimeFromBytes(bytes)||'image/png')}}throw new Error('IMAGE_EMPTY')}
