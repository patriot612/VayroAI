...
async function generateInternal(
  ...
  if(model.provider!=='xkiro'){
    ...
    const isQwenXKiro=model.provider==='xkiro' && model.model_id==='qwen/qwen3.8-max:free';
    if(useWebSearch && !isQwenXKiro){
...
export const openaiCompatAdapter:ProviderAdapter={
  ...
  async generateStream(model:Model,messages:ChatTurn[],env:Env,onDelta:(delta:string)=>Promise<void>,signal?:AbortSignal,options?:AiRequestOptions):Promise<AiResult>{
    ...
    else { const reasoning=options?.reasoning_effort??cfg.reasoning_effort; if(reasoning)body.reasoning_effort=reasoning; const isQwenXKiro=model.model_id==='qwen/qwen3.8-max:free'; if(options?.web_search===true && !isQwenXKiro)body.web_search={enable:true,count:Math.max(1,Math.min(20,Number(options?.web_search_count??cfg.web_search_count??5)))}; }
