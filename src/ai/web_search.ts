...
function sanitizeSearchData(value: string): string {
  // Source documents are untrusted. Remove citation-looking tokens from the data
  // itself so a page cannot smuggle fake [[N]] markers into the model's answer.
  return value.replace(/\[\[\s*\d+\s*\]\]/g, '[source-marker-removed]');
}
...
export function hasSearchEvidence(answer: string, sourceCount: number): boolean {
  ...
  const substantive=paragraphs.filter(p=>p.replace(/[#*`_>\- ]/g,'').length>=40);
  return substantive.length===0 || substantive.every(p=>/\[\[(\d+)\]\]/.test(p));
}
