...
function systemAndContents(messages:ChatTurn[]){
...
export const geminiAdapter:ProviderAdapter={
  async generate(model:Model,messages:ChatTurn[],env:Env):Promise<AiResult>{
    ...
    const cfg=JSON.parse(model.config||'{}') as {max_tokens?:number;temperature?:number;search?:boolean;base_url?:string};
    ...
    const res=await fetch(url,{method:'POST',headers:{'content-type':'application/json','x-goog-api-key':env.GEMINI_API_KEY},body:JSON.stringify(body)});
    const data:any=await res.json();
    if(!res.ok) throw new Error(`GEMINI_${res.status}`);
    const text=(data.candidates?.[0]?.content?.parts??[]).map((p:any)=>p.text??'').join('').trim();
    if(!text) throw new Error('AI_EMPTY_RESPONSE');
    const supports=data.candidates?.[0]?.groundingMetadata?.groundingChunks??[];
    const sources:Source[]=[];
    for(const c of supports){const web=c?.web;if(web?.uri){sources.push({url:web.uri,title:web.title??web.uri});}}
