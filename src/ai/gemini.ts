...
function systemAndContents(messages:ChatTurn[]){const systemMessages=messages.filter(m=>m.role==='system').map(m=>typeof m.content==='string'?m.content:JSON.stringify(m.content??''));const system=systemMessages.length?systemMessages.join('\n\n'):undefined;const contents=messages.filter(m=>m.role!=='system').map(m=>({role:m.role==='assistant'?'model':'user',parts:[{text:typeof m.content==='string'?m.content:JSON.stringify(m.content??'')}]}));return {system,contents};}
...
export const geminiAdapter:ProviderAdapter={
  async generate(model:Model,messages:ChatTurn[],env:Env,signal?:AbortSignal,options?:AiRequestOptions):Promise<AiResult>{
    ...
    if(!options?.search_mode && (model.type==='search'||cfg.search))body.tools=[{google_search:{}}];
