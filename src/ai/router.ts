...
export async function generate(model:Model,messages:ChatTurn[],env:Env,signal?:AbortSignal):Promise<AiResult>{
  const adapter=adapters[model.provider];
  if(!adapter) throw new Error('PROVIDER_NOT_IMPLEMENTED');
  return adapter.generate(model,messages,env,signal);
}
