...
export class TelegramApi{
  ...
  async getFileBytes(fileId:string){const f=await this.getFile(fileId);if(!f.file_path)throw new Error('TELEGRAM_FILE_NOT_FOUND');const res=await fetch(`https://api.telegram.org/file/bot${this.env.TELEGRAM_BOT_TOKEN}/${f.file_path}`);if(!res.ok)throw new Error(`TELEGRAM_FILE_HTTP_${res.status}`);return await res.arrayBuffer()}
