 ...
 if(args[0]==='open'){await save();await screens.imageModelScreen(api,env.DB,env,user,sourceMessageId);return}
  ...
  if (
    ...
    if (args[0] === 'set') {
      ...
      const m=await repo.getSearchModel(env.DB,args[1]);
...
async function handleChatMessage(
  ...
  try {
    ...
    const maxHistoryMessages=Math.max(1,Number((await repo.getSetting(env.DB,'context_max_messages'))??20));
    const maxContextChars=Math.max(1000,Number((await repo.getSetting(env.DB,'context_max_chars'))??24000));
...
async function createNewChat(
  ...
  const activeLimit=Number((await repo.getSetting(env.DB,'active_chat_limit'))??5);
  const activeCount=await repo.countChats(env.DB,user.id,false);
  if(activeCount>=activeLimit){
    await safeSend(api,user.id,t(user.language,'archive_limit_reached'),{reply_markup:ik([[{text:t(user.language,'archive_current_for_new'),callback_data:'chat:archive_current_new'}]])});
    return;
  }
  if (current) {
...
async function handleVisionMessage(text:string,user:any,env:Env,ctx:ExecutionContext,fileId:string){
  ...
  try{
    ...
    holdId=reservation.holdId;
    const status=await api.sendMessage(user.id,t(user.language,'thinking'));progressMessageId=status.message_id;
