...
async function handleImageText(text:string,user:any,env:Env,ctx:ExecutionContext){const api=new TelegramApi(env);const state=await imageSession(env.DB,user.id);if(!state?.modelKey)return false;if(state.awaitingEdit){const gen=await getImageGeneration(env.DB,state.awaitingEdit,user.id);if(gen){await saveImageSession(env.DB,user.id,{...state,awaitingEdit:null});await queueImageGeneration(api,user,env,text,gen.state,gen.resultFileId,gen.telegramMessageId)}return true}if(!state.awaitingPrompt)return false;state.awaitingPrompt=false;if(state.templateId){const pending={...state,pendingPrompt:text};await saveImageSession(env.DB,user.id,pending);const rows=[[{text:t(user.language,'continue_with_template'),callback_data:`image:continue_pending`}],[{text:t(user.language,'without_template'),callback_data:`image:reset_pending`}]];await api.sendMessage(user.id,t(user.language,'image_choice'),{reply_markup:ik(rows)});}else{if(state.lastGenerationId){const old=await getImageGeneration(env.DB,state.lastGenerationId,user.id);if(old?.telegramMessageId){try{await api.deleteMessage(user.id,old.telegramMessageId)}catch{}}}await queueImageGeneration(api,user,env,text,state,state.sourceFileId)}return true}
