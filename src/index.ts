...
async function handleStart(
  ...
  // /start always begins with one fresh technical UI message.
  await clearUiMessage(api, env.DB, user);
  ...
  // The welcome message is the initial technical UI message.
  // It can later be deleted without changing the selected model/role.
  const welcomeMessage = await api.sendMessage(
    user.id,
    t(
      user.language,
      'welcome'
  ...
  await env.DB
    .prepare('UPDATE users SET ui_message_id=? WHERE id=?')
    .bind(welcomeMessage.message_id, user.id)
    .run();

  user.ui_message_id = welcomeMessage.message_id;
  ...
  ctx.waitUntil(
    (async () => {
      ...
      if (
        fresh &&
        current &&
        Number(fresh.ui_message_id ?? 0) === welcomeMessage.message_id
      ) {
...
async function handleCallback(
 ...
 if(args[0]==='open'){await save();await screens.imageModelScreen(api,env.DB,env,user,sourceMessageId);return}
    ...
    if(args[0]==='add'){const key=decodeURIComponent(args[1]);const m=await repo.modelByKey(env.DB,key);const cfg=m?repo.modelConfig(m):{};if(!m||m.type!=='image'||!m.is_active||cfg.supports_input_image!==true)return;await screens.imageAddImageScreen(api,env.DB,env,user,key,state,sourceMessageId);return}
    if(args[0]==='useinput'){const key=decodeURIComponent(args[1]);const m=await repo.modelByKey(env.DB,key);const cfg=m?repo.modelConfig(m):{};if(!m||m.type!=='image'||!m.is_active||cfg.supports_input_image!==true)return;state.hasImage=true;state.mode='image';await env.DB.prepare('UPDATE users SET mode=? WHERE id=?').bind('image',user.id).run();await save();await screens.imageSettingsScreen(api,env.DB,env,user,decodeURIComponent(args[1]),state,sourceMessageId);await safeSend(api,user.id,t(user.language,'send_source_image'));return}
...
async function chatScreenText(api:TelegramApi,user:any,env:Env,sourceMessageId?:number){
  ...
  const text=`💬 <b>${escapeHtml(chat.title)}</b>\n\n🤖 ${escapeHtml(model?.name??chat.model_key)}\n${role}\n\n${t(user.language,'live_messages',{count})}\n\n${user.language==='ru'?'Можно сразу отправить вопрос следующим сообщением.':user.language==='uz'?'Savolni keyingi xabar sifatida yuboring.':'You can send your question in the next message.'}`;
  const rows:any[]=[[{text:t(user.language,'change_model'),callback_data:'models:open'},{text:t(user.language,'new_chat'),callback_data:'chat:new'}],[{text:t(user.language,'tools'),callback_data:'tools:open'}]];
  if(isQwenModel(model))rows.push([{text:t(user.language,'qwen_settings_button'),callback_data:'qwen:settings'}]);
...
async function manualUnarchive(api:TelegramApi,user:any,env:Env,chatId:string){const ok=await repo.switchToArchivedChat(env.DB,chatId,user.id);if(!ok){await safeSend(api,user.id,'⚠️ Нельзя открыть этот архивный диалог. Возможно, достигнут лимит активных диалогов.');return}const fresh=(await repo.getUser(env.DB,user.id))!;await chatScreenText(api,fresh,env)}
...
async function handlePhotoMessage(message:TgMessage,user:any,env:Env,ctx:ExecutionContext){
  ...
  if(user.mode==='image' || state?.hasImage){
    ...
    state.sourceFileId=photo.file_id;state.awaitingPrompt=false;await saveImageSession(env.DB,user.id,state);
    try{const fresh=await repo.getUser(env.DB,user.id);if(fresh)await screens.imageSettingsScreen(api,env.DB,env,fresh,imageModel.model_key,state,fresh.ui_message_id??undefined);}
    catch{}
    return;
  }
...
async function handleImageText(text:string,user:any,env:Env,ctx:ExecutionContext){const api=new TelegramApi(env);const maxChars=Math.max(1,Number((await repo.getSetting(env.DB,'user_message_max_chars'))??8000));if(text.length>maxChars){await safeSend(api,user.id,t(user.language,'too_long'));return true;}const state=await imageSession(env.DB,user.id);if(!state?.modelKey)return false;if(state.awaitingEdit){const gen=await getImageGeneration(env.DB,state.awaitingEdit,user.id);if(gen){await saveImageSession(env.DB,user.id,{...state,awaitingEdit:null});await queueImageGeneration(api,user,env,text,gen.state,gen.resultFileId,gen.telegramMessageId)}return true}if(!state.awaitingPrompt)return false;state.awaitingPrompt=false;if(state.templateId){const pending={...state,pendingPrompt:text};await saveImageSession(env.DB,user.id,pending);const rows=[[{text:t(user.language,'continue_with_template'),callback_data:`image:continue_pending`}],[{text:t(user.language,'without_template'),callback_data:`image:reset_pending`}]];await api.sendMessage(user.id,t(user.language,'image_choice'),{reply_markup:ik(rows)});}else{if(state.lastGenerationId){const old=await getImageGeneration(env.DB,state.lastGenerationId,user.id);if(old?.telegramMessageId){try{await api.deleteMessage(user.id,old.telegramMessageId)}catch{}}}await queueImageGeneration(api,user,env,text,state,state.sourceFileId)}return true}
