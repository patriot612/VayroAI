...
async function handleChatMessage(
  ...
  try {
    ...
    const status=await api.sendMessage(user.id,(qwenSettings?.web_search_enabled===1||fresh.mode==='search')?t(user.language,'searching'):t(user.language,'thinking'));
    ...
    const aiOptions=qwenSettings?{
      reasoning_effort:reasoningEffort(qwenSettings.reasoning_mode),
      web_search:qwenSettings.web_search_enabled===1,
      web_search_count:5,
      streaming:true
    }:undefined;
    let streamedText='';
    ...
    if (
      ...
      let src =
        ...
        result.sources
          .map(
            s =>
              `• <a href="${escapeHtmlAttr(s.url)}">${escapeHtml(s.title)}</a>`
          )
          .join('\n');
...
async function handleVisionMessage(text:string,user:any,env:Env,ctx:ExecutionContext,fileId:string){
  ...
  try{
    ...
    if(result.sources?.length){const src=t(user.language,'sources')+'\n'+result.sources.map(x=>`• <a href="${escapeHtmlAttr(x.url)}">${escapeHtml(x.title)}</a>`).join('\n');await api.sendMessage(user.id,src,{parse_mode:'HTML',disable_web_page_preview:true});}
  }catch(err){
    ...
    const userText=/XKIRO_402/.test(msg)?t(user.language,'qwen_web_quota'):/XKIRO_429/.test(msg)?t(user.language,'qwen_web_unavailable'):/VISION_UNAVAILABLE/.test(msg)?t(user.language,'unsupported_media'):t(user.language,'generic_error');
