...
async function processUpdate(
  ...
  const adminBannerPending=await env.DB.prepare('SELECT * FROM pending_actions WHERE user_id=? AND kind=? AND expires_at>?').bind(user.id,'admin_banner',nowIso()).first<any>();
  if(adminBannerPending && isAdmin(env,user.id) && message.photo?.length){
    const fid=message.photo?.at(-1)?.file_id;
    if(fid){await repo.setSetting(env.DB,'home_banner_file_id',String(fid));await env.DB.prepare('DELETE FROM pending_actions WHERE user_id=? AND kind=?').bind(user.id,'admin_banner').run();await safeSend(api,user.id,t(user.language,'banner_saved'));await admin.adminBanner(api,env.DB,user);return;}
  }
 ...
 if(args[0]==='open'){await save();await screens.imageModelScreen(api,env.DB,env,user,sourceMessageId);return}
  ...
  if (
    ...
    if(args[0]==='banner_upload'){
      await env.DB.prepare('INSERT INTO pending_actions(user_id,kind,payload,expires_at,created_at) VALUES(?,?,?,?,?) ON CONFLICT(user_id) DO UPDATE SET kind=excluded.kind,payload=excluded.payload,expires_at=excluded.expires_at,created_at=excluded.created_at').bind(user.id,'admin_banner',null,isoAfterHours(1),nowIso()).run();
      await safeSend(api,user.id,t(user.language,'banner_prompt')); return;
    }
