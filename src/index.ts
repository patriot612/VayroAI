...
async function queueImageGeneration(api:TelegramApi,user:any,env:Env,prompt:string,state:any,sourceFileId?:string|null,editSourceMessageId?:number){
  ...
  if(!model||model.type!=='image'||!model.is_active||!repo.providerConfigured(model,env))throw new Error('IMAGE_MODEL_UNAVAILABLE');
  const cfg=repo.modelConfig(model);
  if((sourceFileId||state.hasImage) && cfg.supports_input_image!==true)throw new Error('IMAGE_INPUT_NOT_SUPPORTED');
