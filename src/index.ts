...
async function handleCallback(
  ...
  if(action==='chat'&&args[0]==='retry'){const row=await env.DB.prepare('SELECT * FROM pending_actions WHERE user_id=? AND kind=? AND expires_at>?').bind(user.id,'chat_retry',nowIso()).first<any>();if(row){try{const d=JSON.parse(row.payload||'{}');if(d.chatId&&d.modelKey&&d.mode==='chat')await repo.updateChatModel(env.DB,d.chatId,user.id,d.modelKey);await env.DB.prepare('UPDATE users SET mode=?,search_model_key=? WHERE id=?').bind(d.mode||'chat',d.searchModelKey||null,user.id).run();await env.DB.prepare('DELETE FROM pending_actions WHERE user_id=?').bind(user.id).run();const fresh=await repo.getUser(env.DB,user.id);if(fresh)await handleChatMessage(String(d.text??''),fresh,env,{waitUntil(){}} as any);}catch(err){console.error('chat_retry_error',String(err));await safeSend(api,user.id,t(user.language,'generic_error'));}}return;}
 ...
 if(args[0]==='open'){await save();await screens.imageModelScreen(api,env.DB,env,user,sourceMessageId);return}
    ...
    if(args[0]==='cancel'){const changed=await env.DB.prepare("UPDATE image_generations SET status='cancelled' WHERE id=? AND user_id=? AND status='queued'").bind(args[1],user.id).run();if(Number(changed.meta?.changes??0)===1){const row=await env.DB.prepare('SELECT prompt FROM image_generations WHERE id=? AND user_id=?').bind(args[1],user.id).first<any>();if(row){try{const meta=JSON.parse(row.prompt||'{}');if(meta.holdId)await repo.releaseHold(env.DB,meta.holdId)}catch{}}}return}
    if(args[0]==='retry'){
...
async function handleChatMessage(
  ...
  try {
    ...
    const result=await withTimeout(
      ()=>generate(model,messages,env,controller.signal),
...
async function handlePhotoMessage(message:TgMessage,user:any,env:Env,ctx:ExecutionContext){const api=new TelegramApi(env);const state=await imageSession(env.DB,user.id);const model=state?.modelKey?await repo.modelByKey(env.DB,state.modelKey):null;const cfg=model?repo.modelConfig(model):{};if(!state?.hasImage||!model||cfg.supports_input_image!==true){await safeSend(api,user.id,t(user.language,'image_input_required'));return}const photo=message.photo?.[message.photo.length-1];if(!photo)return;state.sourceFileId=photo.file_id;state.awaitingPrompt=false;await saveImageSession(env.DB,user.id,state);try{const fresh=await repo.getUser(env.DB,user.id);if(fresh)await screens.imageSettingsScreen(api,env.DB,env,fresh,model.model_key,state,fresh.ui_message_id??undefined);else if(message.caption?.trim())await safeSend(api,user.id,t(user.language,'image_prompt'));}catch{if(message.caption?.trim())await safeSend(api,user.id,t(user.language,'image_prompt'));}}
