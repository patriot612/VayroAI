...
import { rk } from './telegram/markup';
...
async function handleCallback(
  ...
  if (
    ...
    if (
      ...
      if (chat) {
        ...
        await screens.chatActionsScreen(
          api,
          env.DB,
          user,
          chat,
  ...
  if (
    ...
    if (
      ...
      await screens.pointPlansScreen(
        api,
        env.DB,
        user,
        sourceMessageId
...
async function handleChatMessage(
  ...
  try {
    ...
    const balance =
      await repo.resetFreePointsIfNeeded(
        env.DB,
        user.id
      );

    const reservation =
      await repo.reservePoints(
        env.DB,
        user.id,
        model.cost,
        `ai:${fresh.mode}:${chat.id}:${uuid()}`
      );
...
async function cmdPoints(
  ...
  await env.DB.batch([
    env.DB
      .prepare(
        'UPDATE balances SET purchased_points=MAX(0,purchased_points+?),updated_at=? WHERE user_id=?'
      )
      .bind(
        amount,
        now,
    ...
    env.DB
      .prepare(
        'INSERT INTO transactions(id,user_id,kind,free_amount,paid_amount,ref,created_at) VALUES(?,?,?,?,?,?,?)'
      )
      .bind(
        uuid(),
        id,
  ...
  if (add) {
    ...
    await safeSend(
      api,
      id,
      `✅ <b>Вам начислено ${n} баллов.</b>\n\n` +
      `💰 Текущий баланс: <b>${balance.free + balance.paid}</b> баллов.`
    );
  }
...
async function cmdSetModel(
  ...
  const sql =
    `UPDATE models SET ${field}=? WHERE model_key=?`;
...
async function cmdPrice(
  ...
  await env.DB
    .prepare(
      'UPDATE plans SET price_minor=? WHERE plan_key=?'
    )
    .bind(
      minor,
      key
