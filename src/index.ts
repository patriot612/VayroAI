...
async function handleChatMessage(
  ...
  try {
    ...
    const maxMsgs=Math.max(1,Number((await repo.getSetting(env.DB,'context_max_messages'))??20));
    const maxChars=Math.max(1000,Number((await repo.getSetting(env.DB,'context_max_chars'))??24000));
