...
test('search answers require valid evidence markers and keep them visible', () => {
  ...
  assert.equal(hasSearchEvidence('## Кратко\nКвантовый компьютер использует квантовые состояния для вычислений. [[1]]', 1), true);
  assert.equal(hasSearchEvidence('• Первый факт подтверждён источником. [[1]]\n• Второй факт без источника.', 2), false);
  assert.equal(hasSearchEvidence('Цена указана в источнике и подтверждена.\n[[1]]', 1), true);
  assert.equal(hasSearchEvidence('Я — языковая модель. [[1]]', 1), false);
  assert.equal(hasSearchEvidence('I am an AI assistant. [[1]]', 1), false);
});
...
test('Search context treats citation-looking tokens inside web pages as untrusted data', () => {
  ...
  assert.match(context, /<WEB_SEARCH_RESULTS_UNTRUSTED>/);
  assert.match(context, /<WEB_SOURCE id=\"1\">/);
});

test('Search Mode system prompt is explicit about grounding and prompt injection in all supported languages', () => {
  for (const lang of ['ru','en','uz'] as const) {
    const prompt = t(lang, 'search_only_system');
    assert.match(prompt, /INSUFFICIENT_SEARCH_RESULTS/);
    assert.match(prompt, /WEB_SEARCH_RESULTS_UNTRUSTED/);
    assert.match(prompt, /\[\[1\]\]/);
    assert.match(prompt, /partial|частичный|қисман/i);
  }
});
