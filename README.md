...
Telegram Stars are implemented as the live payment provider for subscription plans. Star prices are stored per plan in D1 and can be changed without redeploying, for example: `/price stars:sub-1m 100`. Telegram sends the successful-payment update back to the bot, and the bot verifies the order, amount, currency and user before activating the subscription. `/paysupport` is enabled for payment issues. Telegram's `telegram_payment_charge_id` is stored so an authorized admin can issue a Stars refund later.

The admin panel now has a `💳 Stars payments` section. From there an admin can select a paid Stars order and choose either `Refund Stars` or `Refund + revoke subscription`. A refund is confirmed through Telegram's `refundStarPayment` API first; the D1 payment/order status is changed only after Telegram confirms the refund. A failed database finalization is left in `refund_pending` for manual review instead of automatically retrying a potentially completed financial operation.

When `feature_payments = 0`, selecting a plan shows that payment is unavailable. `/markpaid` remains available as the existing admin/manual settlement path and is separate from Telegram Stars. The admin panel also has a provider-level switch for Telegram Stars. Turning it off blocks new checkout and pre-checkout while preserving safe settlement of payments that were already completed.

## Telegram update queue

Incoming webhook updates use `processed_updates` as a small durable inbox. New updates enter `processing`, successful handlers move them to `completed`, and failed handlers are stored as `failed` with the last error and attempt count. A `processing` update that has been stale for at least five minutes can be reclaimed once, preventing a worker crash from permanently losing an update. Completed updates are retained for two days; failed ones for seven days.

This protects payment updates as well as callbacks and ordinary user messages from being permanently lost after a transient Worker or D1 failure.

## Tests

Run the built-in scenario tests with:

```bash
npm test
```

The current suite covers update-queue retry decisions, Stars refund safety checks and long Telegram HTML splitting. `npm run typecheck` remains the static TypeScript check.
...
The Worker cron runs every minute and cleans:
...
Image generation uses a D1-backed queue. No Cloudflare Queue resource needs to be created.

The Worker checks the D1 queue every minute, keeps at most 5 image generations running at once, prevents concurrent generations for the same user, and recovers stale jobs after the 5-minute generation timeout.
